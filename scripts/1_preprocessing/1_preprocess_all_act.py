...
def process_amass_act():
    ...
    # try:
    #     processed = [fpath.name for fpath in (config.processed_imu_poser_act / "AMASS").iterdir()] # edit
    # except:
    #     processed = []

    for ds_name in amass_datasets:

        act_file_dict = defaultdict()
        # for act in act_ls:
        #     out_pose, out_shape, out_tran, out_joint, out_vrot, out_vacc = [], [], [], [], [], []

        # print('\nds_name: ', ds_name)
        # print('ds_name_ls: ', ds_name_ls)
        # print('\nprocessed: ', processed)
        # print(ds_name in ds_name_ls)
        # if ds_name in processed:
        #     continue 
        if ds_name not in ds_name_ls:
            continue
        ...
        for npz_fname in tqdm(glob.glob(os.path.join(config.raw_amass_path, ds_name, '*/*_poses.npz'))):
            ...
            print('\nds_dir: ', ds_dir)
            torch.save(out_pose, ds_dir / f'{act_name}_pose.pt')
            torch.save(out_shape, ds_dir / f'{act_name}_shape.pt')
            torch.save(out_tran, ds_dir / f'{act_name}_tran.pt')
            torch.save(out_joint, ds_dir / f'{act_name}_joint.pt')
            torch.save(out_vrot, ds_dir / f'{act_name}_vrot.pt')
            torch.save(out_vacc, ds_dir / f'{act_name}_vacc.pt')
