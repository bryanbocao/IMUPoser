...
def process_amass():
    ...
    print('\nlen(amass_datasets): ', len(amass_datasets))
    print('\nprocessed: ', processed)
    for ds_name in amass_datasets:
        ...
        print('\npose.size(): ', pose.size())
        '''
        length.size():  torch.Size([252])
        shape.size():  torch.Size([252, 10])
        tran.size():  torch.Size([96339, 3])
        pose.size():  torch.Size([96339, 24, 3])
        '''
        www
        ...
        for i, l in tqdm(list(enumerate(length))):
            ...
            print(f'\n pose[b:b + l].size(): {pose[b:b + l].size()}')
            print(f'\n tran[b:b + l].size(): {tran[b:b + l].size()}')
            print(f'\n shape[i].size(): {shape[i].size()}')
            print(f'\n joint[:, :24].contiguous().size(): {joint[:, :24].contiguous().size()}')
            print(f'\n _syn_acc(vert[:, vi_mask]).size(): {_syn_acc(vert[:, vi_mask]).size()}')
            print(f'\n grot[:, ji_mask].size(): {grot[:, ji_mask].size()}')
            print()
            '''
            pose[b:b + l].size(): torch.Size([200, 24, 3])
            tran[b:b + l].size(): torch.Size([200, 3])
            shape[i].size(): torch.Size([10])
            joint[:, :24].contiguous().size(): torch.Size([200, 24, 3])
            _syn_acc(vert[:, vi_mask]).size(): torch.Size([200, 6, 3])
            grot[:, ji_mask].size(): torch.Size([200, 6, 3, 3])
            ...
            pose[b:b + l].size(): torch.Size([150, 24, 3])
            tran[b:b + l].size(): torch.Size([150, 3])
            shape[i].size(): torch.Size([10])
            joint[:, :24].contiguous().size(): torch.Size([150, 24, 3])
            _syn_acc(vert[:, vi_mask]).size(): torch.Size([150, 6, 3])
            grot[:, ji_mask].size(): torch.Size([150, 6, 3, 3])
            '''
        ...
        # debug shapes
        print(f'\nlen(out_pose): {len(out_pose)}')
        for p_ in out_pose:
            print(f'\nnp.shape(p_): ', np.shape(p_))
            '''
            ...
            np.shape(p_):  torch.Size([285, 24, 3])
            np.shape(p_):  torch.Size([640, 24, 3])
            np.shape(p_):  torch.Size([359, 24, 3])
            '''
        print('\nds_dir: ', ds_dir)
        www
