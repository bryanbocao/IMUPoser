...
'''
columns = ['0', '1', '2', '3', '4', '5']
X = polars.DataFrame(X_arr, schema=columns)
y = polars.Series(y_arr)
...
for y_i in range(n_imu):
    print()
    print('\n-----------------------------------------')
    print('\ny_i: ', y_i)
    # y = pd.Series(X[y_i])
    # print('\nnp.shape(y): ', np.shape(y))
    selected_features = mrmr.polars.mrmr_regression(df=X, target_column=str(y_i), K=n_imu, return_scores=True)
